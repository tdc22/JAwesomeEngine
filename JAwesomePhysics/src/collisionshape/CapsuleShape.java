...
public class CapsuleShape extends CollisionShape3 implements CapsuleStructure {
	protected class CapsuleSupport implements SupportCalculator<Vector3f> {
		...
		public CapsuleSupport(CollisionShape<Vector3f, Quaternionf, Quaternionf> cs) {
			collisionshape = cs;
		}
		...
		public Vector3f supportPointLocal(Vector3f direction) {
			...
			float vy = 0;
			float capstart = (float) (halfheight / Math.sqrt(radius * radius + halfheight * halfheight));// VecMath.normalize(new
																											// Vector2f(radius,
																											// halfheight)).y;
