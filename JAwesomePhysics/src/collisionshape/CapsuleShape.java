...
public class CapsuleShape extends CollisionShape3 implements CapsuleStructure {
	protected class CapsuleSupport implements SupportCalculator<Vector3f> {
		...
		public Vector3f supportPointLocal(Vector3f direction) {
			...
			float vy = 0;
			float capstart = (float) (halfheight / Math.sqrt(radius * radius
					+ halfheight * halfheight));// VecMath.normalize(new
												// Vector2f(radius,
												// halfheight)).y;
			if (Math.abs(direction.y) > capstart) {
				float len = (float) new Vector2f(direction.x / capstart,
						direction.z / capstart).length();
				vy = (float) (Math.sqrt(radius * radius - len * len));
			}
			vy += halfheight;
			return new Vector3f(v.x * radius, (v.y < 0 ? -vy : vy), v.z
					* radius);
		}
	...
	public CapsuleShape(float x, float y, float z, float radius,
			float halfheight) {
		super();
		translate(x, y, z);
		this.radius = radius;
