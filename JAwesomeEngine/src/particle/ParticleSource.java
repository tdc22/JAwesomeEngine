...
public class ParticleSource extends GameObject implements Updateable {
	...
	private boolean randomposition = false, randomrate = false,
			radomlife = false;

	public ParticleSource(float x, float y, float z, float spawnrate,
			float lifetime) {
		super();
		translate(x, y, z);
	...
	private void init(Vector3f spawnrate, Vector3f initialVelocity,
			Vector3f gravity, float spawnratetime, float lifetime) {
