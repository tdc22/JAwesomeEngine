...
public class CurveSkeleton2 extends CurveSkeleton<Vector2f, Complexf> {
	...
	List<CurveAnimation<Vector2f, Complexf>> partialAnimations;
	...
	public void updateAnimation() {
		CurveAnimation<Vector2f, Complexf> currentAnimation = getCurrentAnimation();
		for (int i = 0; i < bodyparts.size(); i++) {
			BaseObject<Vector2f, Complexf> part = bodyparts.get(i);
			Vector2f trans = currentAnimation.getAnimationTranslationPath(i).getPoint(currentAnimation.animationTimer);
			if (mirroredX)
				trans.x = -trans.x;
			if (mirroredY)
				trans.y = -trans.y;
			part.translate(trans);
			Complexf rot = currentAnimation.getAnimationRotationPath(i).getRotation(currentAnimation.animationTimer);
			if (invertRotation)
				rot.invert();
			part.rotate(rot);
		}
	...
	public void setDynamicAnimation(CurveAnimation<Vector2f, Complexf> animationparam, float dynamicAnimationSpeed) {
		...
		for (int i = 0; i < bodyparts.size(); i++) {
			Vector2f trans = animation.getAnimationTranslationPath(i).getPoint(animation.animationTimer);
			...
			Complexf newSquadrotation = animation.getAnimationRotationPath(i).getRotation(animation.animationTimer);
