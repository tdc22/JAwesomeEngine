...
public class BoneAnimationSkeleton3 extends BoneAnimationSkeleton<Vector3f, Quaternionf> {
	...
	public BoneAnimationSkeleton3(BoneAnimation<Vector3f, Quaternionf> animation,
			ShapedObject<Vector3f, Quaternionf> shape, BoneJoint rootJoint, int jointCount) {
		super(animation, shape, rootJoint, jointCount);
	}

	public BoneAnimationSkeleton3(BoneAnimation<Vector3f, Quaternionf> animation, BoneJoint rootJoint, int jointCount) {
		super(animation, new ShapedObject3(), rootJoint, jointCount);
	}

	@Override
	protected void interpolate(BoneAnimationKeyframe<Vector3f, Quaternionf> prevKeyframe,
			BoneAnimationKeyframe<Vector3f, Quaternionf> nextKeyframe, float progression) {
		for (int i = 0; i < prevKeyframe.translations.size(); i++) {
			...
			mat.setSubMatrix(QuatMath.slerp(prevKeyframe.rotations.get(i), nextKeyframe.rotations.get(i), progression)
					.toMatrixf());
		}
	}
