...
public class PhysicsDebug {
	...
	private void initAABBObjects() {
		...
		c = Color.GREEN;
		for (GhostObject<Vector3f, Vector3f, Quaternionf, Quaternionf> rb : physics
				.getGhostObjects()) {
			addAABB(rb, c);
		}
		c = Color.BLUE;
		if (physics.getBroadphase() instanceof DynamicAABBTree3) {
			Node3 root = (Node3) ((DynamicAABBTree3) physics.getBroadphase())
					.getRoot();
			if (root != null) {
	...
	private void addAABB(
			RigidBody<Vector3f, Vector3f, Quaternionf, Quaternionf> rb, Color c) {
		AABB<Vector3f> aabb = rb.getAABB();
		ShapedObject3 aabbobj = new ShapedObject3();
		addAABBShape(aabb, aabbobj, c);
		aabbObjects
				.add(new Pair<ShapedObject3, RigidBody<Vector3f, Vector3f, Quaternionf, Quaternionf>>(
						aabbobj, rb));
	}
	...
	private void addAABBShape(AABB<Vector3f> aabb, ShapedObject3 aabbobj,
			Color c) {
		...
		aabbobj.addIndices(0, 1, 0, 2, 0, 3, 1, 4, 1, 5, 2, 4, 2, 6, 3, 6, 3,
				5, 4, 7, 5, 7, 6, 7);
		aabbobj.prerender();
