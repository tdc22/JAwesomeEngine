...
public class VideoSettingsLoader {
		...
		try {
			while ((line = reader.readLine()) != null) {
				if (line.startsWith("Fullscreen"))
					settings.setFullscreen(Boolean
							.parseBoolean(getCleanString(line)));
				if (line.startsWith("Resolution X"))
					settings.setResolutionX(Integer
							.parseInt(getCleanString(line)));
				if (line.startsWith("Resolution Y"))
					settings.setResolutionY(Integer
							.parseInt(getCleanString(line)));
				...
				if (line.startsWith("VSync"))
					settings.setVSync(Boolean
							.parseBoolean(getCleanString(line)));

				if (line.startsWith("Bits per pixel"))
					pixelformat.withBitsPerPixel(Integer
							.parseInt(getCleanString(line)));
				if (line.startsWith("Alpha"))
					pixelformat.withAlphaBits(Integer
							.parseInt(getCleanString(line)));
				if (line.startsWith("Depth"))
					pixelformat.withDepthBits(Integer
							.parseInt(getCleanString(line)));
				if (line.startsWith("Stencil"))
					pixelformat.withSamples(Integer
							.parseInt(getCleanString(line)));
				if (line.startsWith("Samples"))
					pixelformat.withSamples(Integer
							.parseInt(getCleanString(line)));
				if (line.startsWith("Auxiliary"))
					pixelformat.withAuxBuffers(Integer
							.parseInt(getCleanString(line)));
				if (line.startsWith("Accumulation bits per pixel"))
					pixelformat.withAccumulationBitsPerPixel(Integer
							.parseInt(getCleanString(line)));
				if (line.startsWith("Accumulation alpha"))
					pixelformat.withAccumulationAlpha(Integer
							.parseInt(getCleanString(line)));
				if (line.startsWith("Stereo"))
					pixelformat.withStereo(Boolean
							.parseBoolean(getCleanString(line)));
				if (line.startsWith("Floating point"))
					pixelformat.withFloatingPoint(Boolean
							.parseBoolean(getCleanString(line)));
			}
			reader.close();
	...
	public static void save(VideoSettingsOld settings, File file)
			throws IOException {
		...
		writer.write("Bits per pixel: "
				+ settings.getPixelFormat().getBitsPerPixel() + "\n");
		...
		writer.write("Auxiliary: " + settings.getPixelFormat().getAuxBuffers()
				+ "\n");
		writer.write("Accumulation bits per pixel: "
				+ settings.getPixelFormat().getAccumulationBitsPerPixel()
				+ "\n");
		writer.write("Accumulation alpha: "
				+ settings.getPixelFormat().getAccumulationAlpha() + "\n");
		writer.write("Stereo: " + settings.getPixelFormat().isStereo() + "\n");
		writer.write("Floating point: "
				+ settings.getPixelFormat().isFloatingPoint() + "\n");
