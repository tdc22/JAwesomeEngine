...
public class FrictionTest2d extends StandardGame {
	...
	public void init() {
		...
		space = new PhysicsSpace2(new EulerIntegration(), new SAP2(), new GJK2(
				new EPA2()), new SupportRaycast2(), new ImpulseResolution(),
		...
		step = new InputEvent("Step", new Input(Input.KEYBOARD_EVENT, "R",
	...
	public void update(int delta) {
		...
		if (inputs.isKeyDown("E") || step.isActive()) {
			RigidBody2 obj = (RigidBody2) space.getObjects().get(
					space.getObjects().size() - 1);
			System.out.println(obj.getTranslation() + "; " + obj.getRotation()
					+ "; " + space.hasCollision(obj));
			space.update(delta);
		}
		physicsdebug.update();
		cam.update(delta);
