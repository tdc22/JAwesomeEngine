...
public class ManifoldVisualization extends ShapedObject {
	ManifoldVisualization(CollisionManifold<Vector3f> cm) {
		...
		Vector3f ca = VecMath.subtraction(cm.getContactPointA(), cm
				.getObjects().getFirst().getTranslation());
		Vector3f cb = VecMath.subtraction(cm.getContactPointB(), cm
				.getObjects().getSecond().getTranslation());
		Vector3f center1 = VecMath.addition(cm.getObjects().getFirst()
				.getTranslation(), ca);
		Vector3f center2 = VecMath.addition(cm.getObjects().getSecond()
				.getTranslation(), cb);
		Vector3f normal1 = VecMath.subtraction(center1, VecMath.scale(
				cm.getCollisionNormal(), cm.getPenetrationDepth()));
		Vector3f normal2 = VecMath.addition(center2, VecMath.scale(
				cm.getCollisionNormal(), cm.getPenetrationDepth()));
		...
		Vector3f tangent1 = VecMath.addition(center1, VecMath.scale(
				cm.getContactTangentA(), cm.getPenetrationDepth()));
		Vector3f tangent2 = VecMath.addition(center2, VecMath.scale(
